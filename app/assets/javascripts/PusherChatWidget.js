...
function PusherChatWidget(pusher, options) {
  ...
  this._widget.find('button').click(function(event) {
...
/* @private */
PusherChatWidget.prototype._sendChatButtonClicked = function(event) {
  ...
  $.ajax({
    ...
    success: function(result) {
      ...
      var image = $('<div class="pusher-chat-widget-current-user-image">' +
                      '<img src="' + imageInfo.image + '" width="32" height="32" />' +
                    '</div>');
...
PusherChatWidget._createHTML = function(appendTo) {
  ...
  '<div class="pusher-chat-widget">' +
    '<h3 class="text-center chat-title">Add to the Buzz Below</h3>' +
